...
| 14 | **CRM — conformance** | nine `crm.*` / `utm.*` models | **Not a hand-off — a check.** Teh Li Wei built nine CRM worksheets by hand (Phase 2's four, plus the Marketing attribution, Sales Teams and CRM Tags bundles). Read field by field against Odoo on 21 Sep after the owner ruled Odoo the only reference: **21 divergences, 6 that matter** — Leads has no `type` and an invented `Status`, Recurring Plans is an orphan, Campaigns is missing two required relations, Stages' Sales Teams is parked at row 9999, and none of the nine has a roles position. Most of the build is sound, including a correction to our own plan: a lead's tags are `crm.tag`, not Contact Tags | [md](worksheets/14-crm-conformance.md) · page: to publish |
