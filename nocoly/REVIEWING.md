...
| 18 | **CRM Activities** | CRM-scoped `mail.activity` / `mail.activity.type` / `mail.activity.plan` equivalent | Built by Teh Li Wei through CLI. My Activities, Activity Types, Activity Plans and Activity Plan Steps exist; creation, reassignment and due-date notification workflows are published. The workflow-backed Lead `Schedule Activity` Fill action is delivered, creates a real Activity, and resets itself to To-Do/current date/current user after every run; CLI creation and repeated-use plus rendered dialog/Calendar validation passed. Creation notification has rendered evidence; reassignment and 09:00 due delivery still need runtime tests. The hidden Lead subtable remains only as a technical bridge | [md](worksheets/18-crm-activities.md) |
