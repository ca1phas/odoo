...
| 18 | **CRM Activities** | CRM-scoped `mail.activity` / `mail.activity.type` / `mail.activity.plan` equivalent | Built by Teh Li Wei through CLI. My Activities, Activity Types, Activity Plans and Activity Plan Steps exist; creation, reassignment and due-date notification workflows are published. Creation notification has rendered evidence; reassignment and 09:00 due delivery still need runtime tests. The hidden Lead subtable is preserved only as a technical bridge. The Odoo-style Lead top button failed only in rendered Web and was removed, so no broken action remains | [md](worksheets/18-crm-activities.md) |
