...
| 14 | **CRM — conformance** | nine `crm.*` / `utm.*` models | **Not a hand-off — a check.** Teh Li Wei built nine CRM worksheets by hand (Phase 2's four, plus the Marketing attribution, Sales Teams and CRM Tags bundles). Read against Odoo on 21 Sep after the owner ruled Odoo the only reference, then **re-checked against Odoo's own screens**, which withdrew two findings and added a bigger one: **19 divergences, 5 that matter** — no kanban pipeline (the screen Odoo opens on), an invented `Status` on Leads, Recurring Plans built for a feature the tenant has switched off, Campaigns missing two required relations and sitting in a menu Odoo puts under Link Tracker, and no roles position on any of the nine. Much of the build is sound, including a correction to our own plan: a lead's tags are `crm.tag`, not Contact Tags | [md](worksheets/14-crm-conformance.md) · page: to publish |
| 15 | **UI conformance** | 01–07, the seven core worksheets | **Not a hand-off — a check.** The casimir tenant put side by side with ERP Master on 21 Sep 2026. Lists and forms compared on all seven; **buttons, workflows, creation and updates exercised on all seven**. The workflow itself is sound end to end: a document created in the UI took its defaults, drew its Payment Terms from the contact, took Account and the 10 % tax from the product, computed 1,798.00 / 179.80 / 1,977.80 to the cent, and went Draft → Confirm → Posted → Reset to Draft → Cancel → Reset to Draft with the right button on screen at every step. On 01-05 every rule was tripped, every button pressed and a record created and updated: the three Contacts rules, both address workflows and both `_fields_sync` automations; the three Units validations including *Recursion Detected*; the three Products rules; the variant automations and the archive cascade in both directions; and all five Journals rules with both branches of the draft-entry guard. **Five defects**: four controls stay editable on a posted document, *Auto-post until* shows where Odoo hides it, the Journal picker is unfiltered (which is how MISC/2026/00002 got a Misc prefix), Auto-post is offered on receipts, and the journal abort toast reads an untranslated **中止** | [md](worksheets/15-ui-conformance.md) · page: to publish |
| 18 | **CRM Activities** | CRM-scoped `mail.activity` / `mail.activity.type` / `mail.activity.plan` equivalent | Built by Teh Li Wei through CLI. My Activities, Activity Types, Activity Plans and Activity Plan Steps exist; creation, reassignment and due-date notification workflows are published. Lead actions `Schedule Activity` and `Schedule Meeting` both render and feed published Activity-create workflows; the Meeting dialog has start/end, assignee, attendees, location, reminder and API-ready video URL storage. Creation notification has rendered evidence; reassignment and due delivery still need runtime tests. **Do not mark Activity Plan execution, Calendar Event creation or automatic meeting links as done**—the current CLI lacks proven Loop/Create Event schemas | [md](worksheets/18-crm-activities.md) |
