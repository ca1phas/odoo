...
| 15 | Stages | `crm.stage` | **Phase 2 CRM hand-off.** Four stages retained; moved under Configuration by owner and confirmed by CLI/sidebar on 21 Sep. Other conformance findings remain open | [md](worksheets/15-stages.md) |
| 16 | Lost Reasons | `crm.lost.reason` | **Phase 2 CRM hand-off.** Three reasons retained; moved under Configuration by owner and confirmed by CLI/sidebar on 21 Sep | [md](worksheets/16-lost-reasons.md) |
| 17 | Recurring Plans | `crm.recurring.plan` | **Phase 2 CRM hand-off.** Correct empty model; owner hid navigation (`displayType=2`) on 21 Sep. Whether to keep the hidden model is unresolved | [md](worksheets/17-recurring-plans.md) |
| 18 | Leads | `crm.lead` | **Phase 2 CRM hand-off.** Native relations, three views and Won/Lost workflows; CLI reads My Pipeline as grouped view (`viewType=1`, Stage control), contrary to conformance row 14's no-board claim. Fresh UI check blocked by CAPTCHA; current-user filter is static | [md](worksheets/18-leads.md) |

The owner-reported 21 Sep manual corrections, CLI/sidebar evidence, Activities status and unresolved comparison with row 14 are recorded in [the CRM update](CRM-HANDOFF-2026-09-21.md). This update does not constitute final Odoo conformance sign-off.
