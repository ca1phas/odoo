...
| 15 | **UI conformance** | 01–07, the seven core worksheets | **Not a hand-off — a check.** The casimir tenant put side by side with ERP Master on 21 Sep 2026. Lists and forms compared on all seven; **buttons, workflows, creation and updates exercised on 06 Invoices and 07 Invoice Lines only** — that functional pass is still owed on 01–05. The workflow itself is sound end to end: a document created in the UI took its defaults, drew its Payment Terms from the contact, took Account and the 10 % tax from the product, computed 1,798.00 / 179.80 / 1,977.80 to the cent, and went Draft → Confirm → Posted → Reset to Draft → Cancel → Reset to Draft with the right button on screen at every step. **Four defects**: four controls stay editable on a posted document, *Auto-post until* shows where Odoo hides it, the Journal picker is unfiltered (which is how MISC/2026/00002 got a Misc prefix), and Auto-post is offered on receipts | [md](worksheets/15-ui-conformance.md) · page: to publish |
