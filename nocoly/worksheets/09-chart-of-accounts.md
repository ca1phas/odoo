...
| Status | §1 written 16 Sep 2026 · **part A — the worksheet — built and UI-tested 15/15 on 16 Sep 2026** · **part B — the account fields on the other worksheets — built, self-checked and UI-tested 12/13 on 17 Sep 2026**, automation B's journal fallback aligned with Odoo the same day · **28 of 28 pass** (§3 — the role check was run on 18 Sep 2026 with role debugging) · ready for review |
...
| 28 | Field visibility by role | **Pass — run 18 Sep 2026** with HAP's **role debugging** (*Select Role* at the foot of the sidebar), which needs no member. As **Invoicing**: a contact's **Invoicing tab is gone entirely** (its only two fields are hidden, so the empty tab goes with them); a journal shows **no Default Account**, in the list *or* on the form; a product's and a category's **Accounting tab is gone**; Invoice Lines has **no Account column**. As **Accounting Read-only**: the contact's Invoicing tab is gone the same way, but a journal **does** show Default Account — which is the line Odoo draws with `group_account_readonly`. As **Accountant**: the accounts show and Journals offers **+ Record**; as **Invoicing** it does not. The stored role model agrees: `notRead` is true on the five journal accounts for Invoicing and false for Accounting Read-only |
