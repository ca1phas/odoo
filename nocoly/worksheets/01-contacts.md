...
| 11–16 | Street · Street 2 · City · State · ZIP · Country | `street` `street2` `city` `state_id` `zip` `country_id` | Text | no | — | State and Country are dropdowns in Odoo; Text until the Geography bundle. **Country is a Relation → Countries since bundle 4** — see *Added since by bundles* below |
...
| ~~Country~~ and State as dropdowns | **Country was built on 18 Sep 2026 by the Countries bundle** (`worksheets/11-countries.md`): a one-way relation to the Countries worksheet in the text control's cell, carrying all 251 of Odoo's countries, and the text control it replaced was deleted. **State stays Text** until the States bundle (bundle 5), which brings `res.country.state` and Odoo's *State Required* with it |
