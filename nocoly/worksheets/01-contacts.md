...
| 11–16 | Street · Street 2 · City · State · ZIP · Country | `street` `street2` `city` `state_id` `zip` `country_id` | Text | no | — | Street, Street 2, City and ZIP are Text. **State is a Relation → States since bundle 5 and Country a Relation → Countries since bundle 4** — see *Added since by bundles* below |
...
**Added since by bundles:** **Account Receivable** and **Account Payable** (`property_account_receivable_id`, `property_account_payable_id`), relations to Chart of Accounts defaulting to 124000 and 221100, on a new tab **Invoicing** between Sales & Purchase and Notes that is hidden for a contact under a company — bundle 2, `09-chart-of-accounts.md`; **Customer Payment Terms** and **Vendor Payment Terms** (`property_payment_term_id`, `property_supplier_payment_term_id`), relations to Payment Terms on the tab Sales & Purchase — Salesperson \| Customer Payment Terms, then Vendor Payment Terms above the Misc divider — copied from a company that has them and pushed to all of a company's contacts by the two automations below, as Tax ID is — bundle 3, `10-payment-terms.md`; **Country** (`country_id`) is now a one-way Relation → **Countries** (single, dropdown) in the text control's cell — row 9 beside ZIP — carrying all 251 of Odoo's countries, with the Country column on the *Contacts* and *Archived* tables, the Country quick filter and the Kanban card field all pointing at it, and copied between a company and its contacts by the two address automations below like the other five address fields; the Text control it replaced was deleted — bundle 4, `11-countries.md`; **State** (`state_id`) is now a one-way Relation → **States** (single, dropdown) in the text control's cell — row 8 beside City — offering all 2 102 of Odoo's states by Display Name ("Selangor (MY)"), *not* narrowed to the contact's country, copied between a company and its contacts by the two address automations below like the other five address fields, and with **two new workflows, E and F**, that keep it and Country in step the way Odoo's two address onchanges do — E writes the Country from a state that belongs elsewhere, F clears a state whose country is not the contact's, and both are quiet (`triggerType` 2) so neither starts the other; the Text control it replaced was deleted, and **no view of Contacts ever named State**, so none had to be re-pointed — bundle 5, `12-states.md`.
