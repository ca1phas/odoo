...
| 4 | Child Categories | `child_id` | Relation → Product Categories (multiple) — the reverse of 3 | — | — | **Read-only, and hidden on create** ("100"): it is maintained by the children. Odoo has the field and puts it on no view; here it is worth showing, because it is the tree |
...
| 6 | # Products | `product_count` | 汇总 roll-up over field 5, count | — | — | **Read-only, and hidden on create** ("100"). The number on Odoo's stat button. It counts the products **in this category**, not in its children — see the note below |
...
| | # Products — 汇总 (type **37**), count over Products, read-only and hidden on create (**"100"** since 21 Sep 2026) | `6aaa6340805aef7032866556` |
| Fields, the two reverses | Child Categories (the reverse of Parent Category) · Products (the reverse of Category on Products) — both multiple, read-only and hidden on create (**"100"** since 21 Sep 2026) | `6aaa62f67d58b0f44930f095` · `6aaa612ee43d174ab374a1e6` |
| | **Read-only *and hidden on create* (`fieldPermission` "100") since 21 Sep 2026**: at "101" they rendered on the Create Record form, where Odoo — whose smart buttons and one2many lists only exist on a saved record — shows nothing of the kind (`15-ui-conformance.md` §1). "011" would have been wrong: a hidden field never shows as a table column either, and all three are columns of this worksheet's views. | — |
